class GWM_StatusBar : BaseStatusBar
{
	...
	//DrawImage(mtexname, (90, -36), DI_ITEM_OFFSETS|DI_SCREEN_LEFT_BOTTOM);
		...
		if (weapon) // G: we need to rework this later to be a little nicer. use For loops and maybe a value to determine the *shot count*, not *magazine size*
		{
			switch(weapon.GetClassName())
			{
				...
				case 'GWM_Railgun':
					if (weapon.ammocount > 0){DrawImage("2MAG1",(-30,-5));} else {DrawImage("2MAG0",(-30,-5));}
					if (weapon.ammocount > 10){DrawImage("2MAG1",(-40,-5));} else {DrawImage("2MAG0",(-40,-5));}
					if (weapon.ammocount > 20){DrawImage("2MAG1",(-50,-5));} else {DrawImage("2MAG0",(-50,-5));}
					if (weapon.ammocount > 30){DrawImage("2MAG1",(-60,-5));} else {DrawImage("2MAG0",(-60,-5));}
					break;
				default:
